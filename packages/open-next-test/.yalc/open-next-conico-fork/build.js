...
function checkRunningInsideNextjsApp() {
    const extension = ["js", "cjs", "mjs"].find((ext) => fs.existsSync(path.join(appPath, `next.config.${ext}`)));
    if (!extension) {
...
function createServerBundle(monorepoRoot) {
    ...
    const pagesPrerenderedPath = path.join(appPath, ".next/standalone", packagePath, ".next/server/pages");
    if (fs.existsSync(pagesPrerenderedPath)) {
        fs.cpSync(path.join(appPath, ".next/standalone", packagePath, ".next/server/pages"), outputCache, {
            recursive: true,
            verbatimSymlinks: true,
        });
    }
    //Copy app to cache folder
    const appPrerenderedPath = path.join(appPath, ".next/standalone", packagePath, ".next/server/app");
    ...
    if (isMonorepo) {
        fs.writeFileSync(path.join(outputPath, "index.mjs"), [`export * from "./${packagePath}/index.mjs";`].join(""));
    }
}
...
function esbuildSync(options) {
    ...
    if (result.errors.length > 0) {
        ...
        throw new Error(`There was a problem bundling ${options.entryPoints[0]}.`);
    }
}
