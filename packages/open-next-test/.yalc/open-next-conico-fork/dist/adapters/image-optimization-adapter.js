...
import { imageOptimizer, ImageOptimizerCache,
// @ts-ignore
...
// Helper functions //
//////////////////////
function normalizeHeaderKeysToLowercase(headers) {
...
function validateImageParams(headers, queryString) {
    // Next.js checks if external image URL matches the
    // `images.remotePatterns`
    const imageParams = ImageOptimizerCache.validateParams({ headers }, queryString, nextConfig, false);
...
async function optimizeImage(headers, imageParams) {
...
function buildFailureResponse(e) {
    debug(e);
    return {
        statusCode: 500,
        headers: {
